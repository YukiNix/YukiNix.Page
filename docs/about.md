---
layout: page
title: About Me
mathjax: true
permalink: /about/
---

{% include JavaScripts.html %}
{% include mathjax.html %}

<h3>我是谁?</h3>  

&emsp;&emsp;**称呼**: Yuki Nix(禰夕・有希)  
&emsp;&emsp;**目标**: 追问概念的精确解  
&emsp;&emsp;**性别**: 真娘, MtF  
&emsp;&emsp;**工作领域**: 物理科学  
&emsp;&emsp;**精神故乡**: 南极  
&emsp;&emsp;**公共话题**: 涼宮ハルヒ, 岩倉Lain, 長門有希  
&emsp;&emsp;**兴趣**: 阅读(严肃主题), 业余无线电(见下文), 作曲(虚拟歌姬), 语言学习(日本语, 拉丁语)等  
&emsp;&emsp;**教育经历**: 半导体->计算机->理论物理  
&emsp;&emsp;**关键词**: 物理学研究人员・科学理论者・程序设计师・音乐制作人・FPGA 开发者・数字系统工程师・语言学习者・动漫粉丝・业余无线电操作员  
&emsp;&emsp;**拒绝**: 冒犯性的内容(对任何其他实体和我), 广告及推销  

<h3>如果你反复尝试却联系不到我?</h3>  

&emsp;&emsp;点击左上方[**Yuki Nix**](/ "主页")访问我的博客看看如何.  
&emsp;&emsp;主页上能找到我的[**推荐歌单**](/posts/2021/12/22/MusicShare.html "推荐歌单")以及书单[**禰夕百本**](/posts/2026/03/11/BookShelf.html). 听听歌, 读读书, 生活很不错.  
&emsp;&emsp;也可以试试欣赏[「**涼宮ハルヒ**」](https://en.wikipedia.org/wiki/Haruhi_Suzumiya "凉宫春日系列 Wiki")系列与[「**Serial Experiments Lain**」](https://en.wikipedia.org/wiki/Serial_Experiments_Lain "Lain 系列实验 Wiki"), 笔者对它们甚是喜欢.  
&emsp;&emsp;你或许在现实世界里亲手收到了我赠予的一张折纸信封, 如果没有, 请记得(在见面时)向我索取. 其中会有我的移动电话号码, 请将它作为我们的秘密吧.  

<h3>Liaison(联络站)</h3>  

&emsp;&emsp;**无论何处, 彼此相接.**  
&emsp;&emsp;这是一个只供你与我连结的终端界面, 也是从此处开始的最方便的联络工具. 若你担心, 此界面也提供深层网络入口.  

&emsp;&emsp;**公共互联网**: [https://liaison.nixyuki.com](https://liaison.nixyuki.com "联络站地址");  
&emsp;&emsp;**Onion**: [http://yv3tja4tsrkgprubhufw7aa2u47ssz4pbs6dgph74xivrdklfk2ornid.onion](http://yv3tja4tsrkgprubhufw7aa2u47ssz4pbs6dgph74xivrdklfk2ornid.onion "联络站 Onion 地址")  
&emsp;&emsp;**注意**: 这是我自己的设计, 并没有部署在公共或商业设施上. 如果它下线了, 请不要犹豫地切换到别的方式.  

<h3>电子邮件</h3>  

&emsp;&emsp;**不必车马, 及时送达.**  
&emsp;&emsp;只要同一个世界还在同一个互联网, 这是最正式的联络方式. 我会定时查阅, 及时回复.  

&emsp;&emsp;**电子邮件**: [nixyuki@outlook.com](mailto:\\nixyuki@outlook.comm "Send E-Mail to me.")  
&emsp;&emsp;**合适的时间**: 00:00-12:00 UTC;  
&emsp;&emsp;**注意**: 垃圾邮件来源将永久被拒收.  

<h3>无线电</h3>

&emsp;&emsp;**地平线阻挡的, 只是视线.**  
&emsp;&emsp;当基础设施不再可靠, 这可能是此行星上最后的信道. 电台一直开着总有点吵, 平时还是时不时当作休闲好了.  

&emsp;&emsp;**呼号**: BI4AEI  
&emsp;&emsp;**约定频率**:  
&emsp;&emsp;&emsp;&emsp;HF(20m): 14.144 MHz, USB;  
&emsp;&emsp;&emsp;&emsp;VHF(2m): 144.445 MHz, NFW D445N;  
&emsp;&emsp;**备用频率**;  
&emsp;&emsp;&emsp;&emsp;HF(40m): 7.144 MHz, LSB;  
&emsp;&emsp;&emsp;&emsp;HF(10m): 28.144 MHz, USB;  
&emsp;&emsp;&emsp;&emsp;VHF(6m): 51.144 MHz, USB;  
&emsp;&emsp;&emsp;&emsp;UHF(0.6m): 434.445 MHz, D445 N;  
&emsp;&emsp;**工作方式**: Digital, CW;  
&emsp;&emsp;**可能的时间**: 02:00-16:00 UTC;  
&emsp;&emsp;**QSL方式**: eQSL, E-Mail, 纸质 QSL (可联系我寄送纸质卡片. 但出于隐私安全考虑, 我不接收实体卡片);  
&emsp;&emsp;**注意**: 频率上并非总有人值守.  

<h3>即时通讯</h3>  

&emsp;&emsp;**除了快, 只有快.**  
&emsp;&emsp;即时通信是一种数字玩具, 它不是通信的正式途径. 请不要把它当作实时传递消息的工具, 我也只在空闲时才会查阅它们.  

&emsp;&emsp;**Session ID**: 05228a74179d79e613a8cfb96c628dc25b8b9b902487c097bca396db8f1e703b4d;  
&emsp;&emsp;**SimpleX Address**: https://smp9.simplex.im/a#HlsdGyt66EJGlceHUpXFIaFzdETFX404yDhvrf5mAyQ;  
&emsp;&emsp;**注意**: 部分方式需要双方同时在线, 否则会无法送达.  

<h3>移动电话</h3>

&emsp;&emsp;**信息, 如蜂窝密集.**  
&emsp;&emsp;过于及时或许反而会使双方感到困扰, 所以这个部分只提供给熟悉的挚友. 如果我们以前交换过号码, 请通过下面的方法获得新的号码.  

&emsp;&emsp;定义原电话号码: $+86-abb-cdef-ghij$;  
&emsp;&emsp;定义新电话号码: $+86-aqq-rstu-vwxy$;  

&emsp;&emsp;**方法A**:  
&emsp;&emsp;&emsp;&emsp;每位数字变换规则: $y=x(m,n)=[(x+m) \times n] \bmod 10$;  
&emsp;&emsp;&emsp;&emsp;新电话号码: $+86-a(0,1)b(6,3)b(6,3)-c(9,4)d(4,7)e(6,2)f(1,1)-g(2,7)h(9,8)i(4,1)j(4,3)$;  
&emsp;&emsp;**方法B**:  
$$
\begin{vmatrix}q & r & s \\ t & u & v \\ w & x & y \end{vmatrix} = \frac{1}{522} \cdot \begin{vmatrix}b & c & d \\ e & f & g \\ h & i & j \end{vmatrix} \cdot \begin{vmatrix}431 & 189 & -12 \\ -78 & 162 & 288 \\ 271 & -81 & 30 \end{vmatrix} 
$$  
&emsp;&emsp;**外呼专用电话**: $+ba-(w)t-8fre-t448$;  
&emsp;&emsp;**注意**: 短讯息仅接受必要的紧急通告, 通常会忽略语音来电. 仅对外呼叫的号码不接受任何呼入;  

<h3>想要与我交换友情链接吗?</h3>  

&emsp;&emsp;我深知本站非常简陋, 但当作一种设计也可以不是吗? 笑. 如果你不嫌弃的话, 我很乐意与你的站点建立友情链接呢.  
&emsp;&emsp;只要你的页面不过分违反公序良俗, 都请不要犹豫地通过上面的联系方式与我进行讨论呐.  
&emsp;&emsp;本站的信息:  
&emsp;&emsp;&emsp;&emsp;**名称**: Yuki Nix(禰夕・有希)  
&emsp;&emsp;&emsp;&emsp;**地址**: https://www.nixyuki.com  
&emsp;&emsp;&emsp;&emsp;**描述**(如果需要): 追问概念的精确解.  
&emsp;&emsp;&emsp;&emsp;**徽记**(如果需要): https://www.nixyuki.com/include/logo_w.jpg  
&emsp;&emsp;&emsp;&emsp;**描述**(如你需要英文版本): In pursuit of the precise resolution of concepts.  

&emsp;&emsp;顺颂  
春祉  

<p align="right">Yuki N.</p>
<p align="right">2026.03.20</p>